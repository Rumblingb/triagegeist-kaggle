#!/usr/bin/env python3
"""LightGBM — single model, 2-fold, 300 estimators"""
...
def fe(df):
    ...
    d['ppr']=d['pulse_pressure']/(d['systolic_bp']+1e-6);d['mbr']=d['mean_arterial_pressure']/(d['systolic_bp']+1e-6);d['sf']=d['spo2']/100.0;d['hrr']=d['heart_rate']/(d['respiratory_rate']+1e-6);d['bo']=d['bmi']*d['spo2']/100.0;d['hxt']=d['heart_rate']*d['temperature_c']/100.0
    d['n2r']=(d['respiratory_rate']>20).astype(int)+(d['respiratory_rate']>24).astype(int);d['n2o']=(d['spo2']<96).astype(int)+(d['spo2']<94).astype(int)+(d['spo2']<92).astype(int);d['n2c']=(d['gcs_total']<15).astype(int);d['n2h']=(d['heart_rate']>=91).astype(int)+(d['heart_rate']>=111).astype(int)
    d['ahr']=d['age']*d['heart_rate']/100;d['arr']=d['age']*d['respiratory_rate']/100;d['asbp']=d['age']*d['systolic_bp']/100;d['ash']=d['age']*d['shock_index'];d['an2']=d['age']*d['news2_score']/100
    hx=[c for c in d.columns if c.startswith('hx_')];d['thx']=d[hx].sum(axis=1);d['cr']=((d['hx_heart_failure']>0)&(d['hx_ckd']>0)).astype(int)
    d['hs']=np.sin(2*np.pi*d['arrival_hour']/24);d['hc']=np.cos(2*np.pi*d['arrival_hour']/24);d['ms']=np.sin(2*np.pi*d['arrival_month']/12);d['mc']=np.cos(2*np.pi*d['arrival_month']/12)
    return d
...
print(f"Data: {X.shape}, {Xt.shape}",flush=True)
...
for f,(tr,va) in enumerate(skf.split(X,y)):
    print(f"  LGB Fold {f+1}/2...",flush=True)
    m=lgb.LGBMClassifier(n_estimators=300,learning_rate=0.08,max_depth=10,num_leaves=63,
        subsample=0.85,colsample_bytree=0.85,class_weight='balanced',
        reg_alpha=0.1,reg_lambda=0.1,min_child_samples=20,
        random_state=42,verbose=-1,n_jobs=-1)
    m.fit(X[tr],y[tr])
    lgb_oof[va]=m.predict_proba(X[va])
    lgb_test+=m.predict_proba(Xt)/2
    print(f"  Fold {f+1} done",flush=True)

preds=classes[np.argmax(lgb_oof,axis=1)]
mf1=float(f1_score(y,preds,average='macro'))
print(f"\nLightGBM CV MF1: {mf1:.4f}",flush=True)

# Also train on full data for submission
print("Training full model...",flush=True)
final=lgb.LGBMClassifier(n_estimators=500,learning_rate=0.05,max_depth=10,num_leaves=63,
    subsample=0.85,colsample_bytree=0.85,class_weight='balanced',
    reg_alpha=0.1,reg_lambda=0.1,min_child_samples=20,
    random_state=42,verbose=-1,n_jobs=-1)
final.fit(X,y)
test_preds=classes[np.argmax(final.predict_proba(Xt),axis=1)]
sub=pd.DataFrame({'patient_id':test['patient_id'].values,'triage_acuity':test_preds})
sub.to_csv('/root/triagegeist-kaggle/artifacts/lgb_submission.csv',index=False)
print(f"Submission saved. Dist: {dict(zip(*np.unique(test_preds,return_counts=True)))}",flush=True)

with open('/root/triagegeist-kaggle/artifacts/lgb_results.json','w') as f:
    json.dump({'cv_mf1':mf1,'submission':'artifacts/lgb_submission.csv'},f)
print(f"DONE! LightGBM: {mf1:.4f}",flush=True)
